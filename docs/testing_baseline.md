...
## Q02 isolated multi-turn evaluation (2026-09-24)

The offline Docker baseline passes **145 tests, zero failures/errors/skips**. Seven new contracts cover interleaved conversations and fresh-run separation through runner/HTTP, unchanged ordinary history with connector access prohibited, rejection of privileged roles and malformed/oversized context, failure propagation without losing denominator entries, complete-pair context bounds, oversized response handling, dataset ordering and follow-up context in the real graph's rewrite/synthesis prompts. Providers remain scripted: these prove context plumbing and deterministic accounting, not live-model reasoning quality.

Blocked turns record null latency because no request occurred. Existing single-turn datasets and ordinary chat contracts continue passing. Evaluation context is ephemeral; interrupted-run recovery and authorization remain separate tasks. Verification command: `docker compose -f compose.test.yml run --rm --no-deps baseline`. Q01 passed all GitHub jobs in [run 36002572085](https://github.com/Jim-lan/log-pilot/actions/runs/36002572085); Q02 remote verification follows its commit.


## Q03 versioned quality fixtures and compound SQL filters (2026-09-24)
