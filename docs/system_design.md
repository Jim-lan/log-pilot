...
| Retrieval and runbooks | LlamaIndex/Chroma retrieval, log-template knowledge and Markdown topic synthesis | Original document spans/versioning and durable Markdown replay remain open |
...
| File ingestion | Immutable-file contract, content fingerprint, SQLite acknowledgement ledger, quarantine and successful-only acknowledgement | Append/tail sources unsupported; queue bounds and distributed ownership absent |
| Log recovery | Atomic DuckDB rows/event keys/index outbox; deterministic vector upsert; explicit protocol-2 replay skips committed lines | Recover older pending files before newer pattern versions; legacy vectors and Markdown need separate migration |
