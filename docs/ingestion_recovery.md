...
## R07: do not overtake unfinished log indexing

Before admitting a new log claim, the SQLite ledger rejects any older incomplete claim. Explicit replay must select the oldest incomplete log claim (insertion order), while an already-indexed retry may still finish its file move. A second check against DuckDB's outbox rejects outstanding pattern work belonging to another file, including orphaned work whose ledger entry is missing. Ordinary watcher startup checks for unresolved ledger/outbox work and requires explicit recovery before resuming intake.
...
## R09: activity-based retention planning

New stable pattern upserts record numeric UTC `first_seen_at_unix`, `last_seen_at_unix` and `last_indexed_at_unix`, with `retention_schema=1`. Aware timestamps normalize to UTC; naive log timestamps are interpreted as UTC under this local adapter contract, not as proof of the producer's original timezone. Existing first/last event bounds are merged monotonically, and last-indexed time never regresses. Stable pattern IDs remain unchanged. Activity bookkeeping is excluded from embedding/LLM metadata inputs.

A dry-run retention candidate must be a log pattern with valid schema, no retention hold, last event strictly older than the cutoff AND last indexing strictly older than the cutoff. Equality is retained. Recently replayed historical files therefore retain their patterns. Missing/invalid legacy age or unknown activity schema produces a hold; legacy records without verified activity and all runbook cards are excluded from this cleanup policy. Raw sources, document snapshots, journals, history and evaluation need their own O03 retention rules.

`KnowledgeStore.retention_candidates(cutoff)` and `Janitor.run_cleanup(days)` now return bounded dry-run reports containing candidate ID hashes, counts and `mutations: 0`. The legacy `delete_older_than` entry point raises instead of deleting anything. No cleanup runs automatically at worker startup. Deletion remains disabled until O03/O04 establish a reviewed retention policy and exercised restore procedure. Reports require stable/offline storage and are not authorization to delete; count checks do not establish snapshot isolation against concurrent writers.

The read/merge/upsert activity update relies on the existing single-owner ingestion contract and R07 ordering. It is not an atomic multi-writer maximum operation. Tests cover late events, recent replay, exact cutoff boundaries, UTC offsets, invalid/held metadata, non-pattern exclusions and bounded reports. A real-vector smoke verifies monotonic metadata and that the production deletion entry point cannot remove records.
