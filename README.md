...
Durable ingestion, bounded intake and isolated multi-turn evaluation now have regression coverage. Versioned synthetic quality fixtures cover SQL, source facts and abstention. Next designs address live-model evaluation, identity/tenant isolation, storage ownership and operational qualification. No authentication or tenant isolation exists today. Model names in configuration are identifiers, not verified quality/hardware recommendations. Cloud inference and opt-in web search can send content externally despite best-effort redaction.
